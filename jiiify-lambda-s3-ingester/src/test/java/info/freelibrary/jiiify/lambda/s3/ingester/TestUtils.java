...
public class TestUtils {
    ...
    private static final ObjectMapper SNS_EVENT_MAPPER = new ObjectMapper();

    private static final ObjectMapper DYNAMODB_EVENT_MAPPER = new ObjectMapper();
    ...
    static {
        ...
        SNS_EVENT_MAPPER.configure(JsonParser.Feature.ALLOW_COMMENTS, true);
        SNS_EVENT_MAPPER.setPropertyNamingStrategy(PropertyNamingStrategy.UPPER_CAMEL_CASE);
        SNS_EVENT_MAPPER.registerModule(new TestJacksonMapperModule());

        DYNAMODB_EVENT_MAPPER.configure(JsonParser.Feature.ALLOW_COMMENTS, true);
        DYNAMODB_EVENT_MAPPER.setPropertyNamingStrategy(new UpperCaseRecordsPropertyNamingStrategy());
        DYNAMODB_EVENT_MAPPER.registerModule(new TestJacksonMapperModule());

        DYNAMODB_EVENT_MAPPER.addMixIn(Record.class, DynamodbEventMixin.RecordMixin.class);
        DYNAMODB_EVENT_MAPPER.addMixIn(StreamRecord.class, DynamodbEventMixin.StreamRecordMixin.class);
        DYNAMODB_EVENT_MAPPER.addMixIn(AttributeValue.class, DynamodbEventMixin.AttributeValueMixIn.class);
    }

    private static final DateTimeFormatter DATETIME_FORMATTER = ISODateTimeFormat.dateTime().withZone(
    ...
    public static <T> T parse(final String aResource, final Class<T> aClass) throws IOException {
        ...
        try {
            if (aClass == S3Event.class) {
                ...
                final T result = (T) new S3Event(event.getRecords());
                return result;

            } else if (aClass == SNSEvent.class) {
                return SNS_EVENT_MAPPER.readValue(stream, aClass);
            } else if (aClass == DynamodbEvent.class) {
                return DYNAMODB_EVENT_MAPPER.readValue(stream, aClass);
            } else {
    ...
    private static class DateTimeSerializer extends JsonSerializer<DateTime> {
        ...
        @Override
        public void serialize(final DateTime aValue, final JsonGenerator aJsonGen, final SerializerProvider aProvider)
                throws IOException {

            aJsonGen.writeString(DATETIME_FORMATTER.print(aValue));
        }
    }
    ...
    private static class DateTimeDeserializer extends JsonDeserializer<DateTime> {
        ...
        @Override
        public DateTime deserialize(final JsonParser aJsonParser, final DeserializationContext aContext)
                throws IOException {

            return DATETIME_FORMATTER.parseDateTime(aJsonParser.getText());
        }
    }

    private static class UpperCaseRecordsPropertyNamingStrategy extends
            PropertyNamingStrategy.PropertyNamingStrategyBase {

        private static final long serialVersionUID = 1L;

        @Override
        public String translate(final String aPropertyName) {
            if (aPropertyName.equals("records")) {
                return RECORDS;
            }
